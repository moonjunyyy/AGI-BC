...
class ETRI22_2S_Video_Generation_Dataset(Dataset):
    def __init__(
        ...
        self.dataframe = pd.read_csv(
            os.path.join(self.path, "etri2022_end.tsv"), sep="\t", index_col=0
        )
        ...
        self.NoBC_train_dataframe = self.NoBC_dataframe.sample(
            frac=0.8, random_state=42
        )
        self.NoBC_test_dataframe = self.NoBC_dataframe.drop(
            self.NoBC_train_dataframe.index
        )
        """
        ...
        if self.train:
            self.dataframe = pd.concat(
                [self.BC_train_dataframe, self.NoBC_train_dataframe]
            ).sample(frac=1, random_state=42)
        else:
            self.dataframe = pd.concat(
                [self.BC_test_dataframe, self.NoBC_test_dataframe]
            ).sample(frac=1, random_state=42)
    ...
    def __getitem__(self, index):
        ...
        if audio.size(1) != int(sr * self.length):
            audio = F.pad(
                audio, (0, int(sr * self.length) - audio.size(1)), "constant", 0
            )
        if audio.size(0) != 1:
        ...
        if target_audio.size(1) != int(self.predict_length * sr):
            target_audio = F.pad(
                target_audio,
                (0, int(sr * self.predict_length) - target_audio.size(1)),
                "constant",
                0,
            )
        ...
        target_indices = np.linspace(
            start=length - int(fps * 0.5), stop=length - 1, num=8, endpoint=False
        )
...
class ETRI23_2S_Video_Generation_Dataset(Dataset):
    def __init__(
        ...
        self.dataframe = pd.read_csv(
            os.path.join(self.path, "etri2023_end.tsv"), sep="\t", index_col=0
        )
        self.dataframe = self.dataframe.assign(filename=range(len(self.dataframe)))
...
class ETRI_ALL_2S_Video_Dataset(Dataset):
    def __init__(
        ...
        self.dataset_2022 = ETRI22_2S_Video_Generation_Dataset(
            path, tokenizer, train, balanced, length, predict_length
        )
        self.dataset_2023 = ETRI23_2S_Video_Generation_Dataset(
            path, tokenizer, train, balanced, length, predict_length
        )
    ...
    def get_sample_in_class(self):
        return (
            self.dataset_2022.get_sample_in_class()
            + self.dataset_2023.get_sample_in_class()
        )
