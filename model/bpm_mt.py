...
# BPM_ST is the model for the single task learning
class BPM_ST_Target(nn.Module):
    def __init__(self, language_model=None, audio_model=None, output_size=128, num_class=4, sentiment_output_size=64, dropout=0.3, mode="cross_entropy"):
        super(BPM_ST_Target, self).__init__()
...
# BPM_ST is the model for the single task learning
class BPM_ST_Target_Token(nn.Module):
    def __init__(self, language_model=None, audio_model=None, output_size=128, num_class=4, sentiment_output_size=64, dropout=0.3, mode="cross_entropy"):
        super(BPM_ST_Target_Token, self).__init__()
    ...
    def forward(self, x):
        ...
        if self.mode != "audio_only":
            text = self. language_model.embeddings(text).to(device)
            target_text = self. language_model.embeddings(target_text).to(device)
            text = torch.cat((text, target_text), dim=1)
