...
class Contrastive_Prototype(nn.Module):
    ...
    def pretext_forward(self, dataloader):
        ...
        ### Below is for clustering from input data. ###
        ...
        audio_key_matcher_optimizer = torch.optim.Adam(self.audio_key_matcher.parameters(), lr=1e-3)
        text_key_matcher_optimizer = torch.optim.Adam(self.text_key_matcher.parameters(), lr=1e-3)
        ...
        while train_audio or train_text:
            ...
            text_key_matcher_optimizer.zero_grad()
            if train_audio:
            ...
            if train_text:
                ...
                if text_acc.item() > 0.95:
                    train_text = False
            print(f"loss: {audio_loss.item():.4f}, {text_loss.item():.4f}, audio_acc: {audio_acc.item():.4f}, text_acc: {text_acc.item():.4f}", end='\r')
        print()

        for name, param in self.audio_key_matcher.named_parameters():
            param.requires_grad = False
        for name, param in self.text_key_matcher.named_parameters():
            param.requires_grad = False
    ...
    def forward(self, x):
        ...
        # target_audio = _in[:, torch.randint(0, _in.shape[1], (2,))].flatten(1,2)
        # if self.multi_modal:
        #     _in = self.audio_text_centriods[cluster]
        #     target_audio_text = _in[:, torch.randint(0, _in.shape[1], (2,))]
            
            # audio_selection = self.audio_key_matcher(_audio).argmax(dim=1)
            # target_audio = self.audio_target_centriods[audio_selection]

            audio_weight = self.audio_key_matcher(_audio).softmax(dim=1)
        ...
        # target_text = _in[:, torch.randint(0, _in.shape[1], (2,))].flatten(1,2)
        # if self.multi_modal:
        #     _in = self.text_audio_centriods[cluster]
        #     target_text_audio = _in[:, torch.randint(0, _in.shape[1], (2,))]
            
            # text_selection = self.text_key_matcher(_text).argmax(dim=1)
            # target_text = self.text_target_centriods[text_selection]

            text_weight = self.text_key_matcher(_text).softmax(dim=1)
