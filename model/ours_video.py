...
class Ours_Video(nn.Module):
    def __init__(self, language_model=None, audio_model=None, video_model=None, sentiment_dict = None, output_size=128, num_class=4, sentiment_output_size=64, dropout=0.3, mode="cross_entropy"):
        ...
        bert_params = list(self.language_model.parameters()) + list(self.audio_model.parameters()) + list(self.video_model.parameters())
        self.align_optimizer = torch.optim.Adam(bert_params, lr=1e-5)
        self.align_optimizer.add_param_group({"params": 
                                              list(self.cross_attn0.parameters()) + list(self.cross_attn1.parameters()) + list(self.cross_attn2.parameters()) +
                                              list(self.audio_downproject.parameters()) + list(self.text_downproject.parameters()) + list(self.video_downproject.parameters())})
    ...
    def forward(self, x, train="train"):
        ...
        for l, (a_layer, t_layer, v_layer) in enumerate(zip(a_layers, t_layers, v_layers)):
            ...
            if l > 8:
                ...
                if MODE == "ALL" or MODE == "AT":
                    a_t = self.cross_attn0[-1](a_feature, t_feature)
                if MODE == "ALL" or MODE == "AV":
                    a_v = self.cross_attn1[-1](a_feature, v_feature)
                if MODE == "ALL" or MODE == "TV":
                    t_v = self.cross_attn2[-1](t_feature, v_feature)
                if MODE == "TA":
                    t_a = self.cross_attn0[-1](a_feature, t_feature)
                if MODE == "VA":
                    v_a = self.cross_attn1[-1](v_feature, a_feature)
                if MODE == "VT":
                    v_t = self.cross_attn2[-1](v_feature, t_feature)
