...
MODE = "ALL"
class Ours_Video_Align(nn.Module):
    def __init__(self, language_model=None, audio_model=None, video_model=None, sentiment_dict = None, output_size=128, num_class=4, sentiment_output_size=64, dropout=0.3, mode="cross_entropy"):
        super(Ours_Video_Align, self).__init__()
        ...
        # self.cross_attn0 = nn.Sequential(*[CrossAttentionLayer(d_query=192, d_kv=192, nhead=12) for _ in range(1)])
        # self.cross_attn1 = nn.Sequential(*[CrossAttentionLayer(d_query=192, d_kv=192, nhead=12) for _ in range(1)])
        # self.cross_attn2 = nn.Sequential(*[CrossAttentionLayer(d_query=192, d_kv=192, nhead=12) for _ in range(1)])
        # self.audio_downproject = nn.Linear(768, 192) 
        # self.text_downproject = nn.Linear(768, 192) 
        # self.video_downproject = nn.Linear(384, 192)

        # for name, param in self.named_parameters():
        #     if "audio_model" in name or "language_model" in name or "video_model" in name:
        #         param.requires_grad = False
        #     if "cross_attn" in name or "downproject" in name:
        #         param.requires_grad = False
        ...
        for l, (a_layer, t_layer, v_layer) in enumerate(zip(a_layers, t_layers, v_layers)):
            ...
            if l > 8:
                ...
                y["InfoNCE"] += ( self.contrastive_loss(self.sim_matrix(a_feature, t_feature))
                                  + self.contrastive_loss(self.sim_matrix(a_feature, v_feature))
                                  + self.contrastive_loss(self.sim_matrix(t_feature, v_feature))
                                  + self.contrastive_loss(self.sim_matrix(t_a, v_feature))
                                  + self.contrastive_loss(self.sim_matrix(v_a, t_feature))
                                  + self.contrastive_loss(self.sim_matrix(v_t, a_feature)))
