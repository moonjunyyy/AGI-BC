...
Kmeans_alloc(PyTypeObject *type, Py_ssize_t nitems)
    {auto ret = PyObject_New(KMeans, type); if (ret == NULL) return NULL; ret->x_attr = NULL; return (PyObject*)ret;}
...
Kmeans_dealloc(KMeans *self)
    {Py_XDECREF(self->x_attr); Py_TYPE(self)->tp_free((PyObject *)self);}

void 
Kmeans_finalize(KMeans *self)
    {if (self->centroids != nullptr) {self->centroids.reset(); self->labels.reset(); self->distances.reset(); self->mask.reset();}}

int 
Kmeans_setattr(KMeans *self, const char *name, PyObject *v)
    {if (self->x_attr == NULL) {
...
// PyObject *
// new_Kmeans(PyObject *self, PyObject *args)
// { KMeans *kmean = (KMeans*)Kmeans_alloc(&KMeansType,0); if (kmean != NULL) Kmeans_init(kmean, args); return (PyObject*)kmean;}

#ifdef __cplusplus
}
#endif

// char * c_str(PyObject *o) {char* c; auto length = PyUnicode_GetLength(o); PyBytes_AsStringAndSize(PyUnicode_AsUTF8String(o), &c, &length); return c;}
