...
class _WavFile:
    ...
    def _get_metadata(self):
        with open(self.filename, "rb") as file:
            ...
            while True:
                ...
                if not subchunk_id:
                    raise ValueError(
                        "Unexpected end of file while searching for fmt subchunk"
                    )
            ...
            assert (self.byte_rate) == (
                self.sample_rate * self.channels * self.bits_per_sample // 8
            )
            ...
            while True:
                ...
                if not subchunk_id:
                    raise ValueError(
                        "Unexpected end of file while searching for data subchunk"
                    )
    ...
    def __getitem__(self, idx):
        ...
        if self.bits_per_sample == 8:
            dtype = np.dtype(np.uint8)  # 8비트 오디오는 일반적으로 부호 없는 데이터
        elif self.bits_per_sample == 16:
            dtype = np.dtype(np.int16)
        elif self.bits_per_sample == 32:
            dtype = np.dtype(np.int32)
        else:
            raise NotImplementedError("Unsupported bit depth")
        ...
        if dtype == np.uint8:
            data = (
                data - 128
            ) / 128.0  # 8비트 오디오는 0-255 범위이므로 중앙값을 기준으로 조정
        elif dtype == np.int16:
            data = data / (2**15)  # 16비트 오디오는 -2^15 ~ 2^15-1 범위
        elif dtype == np.int32:
            data = data / (2**31)  # 32비트 오디오는 -2^31 ~ 2^31-1 범위
...
class WavFile:
    ...
    def __del__(self):
        self._manager._instance._release_file(self.filename)
