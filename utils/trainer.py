...
from utils.utils import (
    ...
    get_audio_model,
    get_language_model,
    get_backchannel_prediction_model,
    get_video_model,
)
from utils.criterions import get_criterion
from m00nny_utils.torch_util.lr_scheduler.warmup_cosine_anneling import (
    WarmUpCosineAnnelingScheduler,
)
...
class Trainer:
    def __init__(self, args) -> None:
        ...
        print(
            f"world_size: {self.world_size}, rank: {self.rank}, ngpus_per_node: {
              self.ngpus_per_node}, distributed: {self.distributed}"
        )
    ...
    def _run(self, rank, world_size):
        ...
        self.train_dataset, self.val_dataset, self.num_class = get_dataset(
            self.dataset, self.data_path, tokenizer
        )
        ...
        try:
            state_dict = torch.load(
                f"{self.path}/pretrained.pt", map_location=f"cpu", weights_only=True
            )
        ...
        for epoch in range(self.epochs):
            ...
            for b, batch in enumerate(self.train_dataloader):
                ...
                with torch.amp.autocast("cuda", dtype=torch.float16):
                    ...
                    if "logit" in y.keys():
                        ...
                        accuracy = (
                            (logit.argmax(dim=-1) == batch["label"]).float().mean()
                        )
                    ...
                    if self.is_MT:
                        loss = 0.9 * loss + 0.1 * F.cross_entropy(
                            y["sentiment"], batch["sentiment"], reduction="mean"
                        )
                    if "audio" in y.keys():
                ...
                gc.collect()
                print(
                    f"Epoch : {epoch}, {b}/{len(self.train_dataloader)}, Loss : {
                      loss.item():.6f}, Acc : {accuracy.item()*100:.3f}",
                    end="\r",
                )
            print(flush=True)
            gc.collect()
            ...
            with torch.no_grad():
                ...
                loss = 0
                tp = torch.tensor(
                    [0 for _ in range(self.num_class)], device=self.local_rank
                )
                fp = torch.tensor(
                    [0 for _ in range(self.num_class)], device=self.local_rank
                )
                ...
                for i, batch in enumerate(self.val_dataloader):
                    ...
                    accuracy += (
                        (torch.argmax(logit, dim=1) == batch["label"]).float().sum()
                    )
                ...
                weighted_f1_score = (
                    f1_score * number_of_classes
                ).sum() / number_of_classes.sum()
                print(
                    f"Epoch : {epoch}, Loss : {loss.item():.6f}, Train Loss : {train_loss:.6f}, Train Acc : {train_acc*100:.3f},\nAcc : {
                      accuracy.item()*100:.3f}, Loss : {loss.item():.6f}, Weighted F1 : {weighted_f1_score.item()*100:.3f}, F1 : ",
                    *(f1_score * 100).cpu().tolist(),
                )
        ...
        if self.distributed:
            if torch.cuda.is_available():
                ...
                if self.distributed:
                    ...
                    print(
                        f"rank {self.rank}/{self.world_size} is running on GPU {self.device}"
                    )
