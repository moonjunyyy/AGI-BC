...
import torch
# CUDAGraphed from moshi/utils/compile.py is inlined here as a simplified stub.
...
class State(abc.ABC):
    ...
    def __post_init__(self):
        ...
        self._set_exec_mask_graphed = None  # stub: was CUDAGraphed in original
...
class StreamingModule(abc.ABC, nn.Module, tp.Generic[StateT]):
    ...
    def set_exec_mask(self, exec_mask: torch.Tensor):
        ...
        def _set_exec_mask_fn(name: str, module: 'StreamingModule'):
            s = module._streaming_state
            assert s is not None
            s.set_exec_mask(exec_mask)
